<div class="submenu">     
    <div class="menu"><a href="#sede">SEDE</a></div>
    <div class="menu"><a href="#ponentes">PROGRAMA</a></div>
    <div class="menu"><a href="#hospedaje">HOSPEDAJE</a></div>
<!--
    <div class="menu"><a href="programa.php">PROGRAMA</a></div>
-->
    <div class="menu"><a href="#expomf">EXPOMF18</a></div>
    <div class="menu"><a href="#pronafim">PRONAFIM</a></div>
    <div class="menu"><a href="https://acob.guru/e-nscribe/iniciopronafim2018.asp" class="registrate">REGÍSTRATE AQUÍ</a></div>
</div>
